...
export default function CargarTurno() {
  ...
  const okColor   = '#27500A'
  const failColor = '#A32D2D'
    ...
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '1rem' }}>
      ...
      <div style={{ background: '#fff', border: '0.5px solid #e0ddd4', borderRadius: 12, padding: '1rem 1.25rem', marginBottom: '1rem' }}>
        ...
        <div style={{ marginTop: '0.75rem', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <span style={chip('#EEEDFE', '#3C3489')}>x{mult.toFixed(1)} multiplicador</span>
          {fds && <span style={chip('#FAEEDA', '#633806')}>Fin de semana — x2.0 aplicado</span>}
          {ptsBase > 0 && <span style={chip('#EAF3DE', '#27500A')}>{ptsTotal.toFixed(0)} pts totales del turno</span>}
        </div>
      </div>
      ...
      <div style={{ background: '#fff', border: '0.5px solid #e0ddd4', borderRadius: 12, padding: '1rem 1.25rem', marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>Obj ≤ {OBJ.completo}%</div>
          </div>
          <div>
            ...
            <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>Obj ≤ {OBJ.incompleto}%</div>
          </div>
          <div>
            ...
            <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>Obj 35"–50"</div>
          </div>
        </div>
      </div>
      ...
      {/* Stat diario */}
      <div style={{ background: '#fff', border: '0.5px solid #e0ddd4', borderRadius: 12, padding: '1rem 1.25rem', marginBottom: '1rem' }}>
        <div style={{ fontSize: 13, fontWeight: 500, marginBottom: '0.75rem' }}>Stat del día <span style={{ fontSize: 11, fontWeight: 400, color: '#888' }}>(cargar una vez por día)</span></div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>Obj: entre −0.10% y +0.10%</div>
          </div>
        </div>
      </div>
      ...
      {puedeAplicarPenal && (
        <div style={{ background: '#fff', border: '0.5px solid #e0ddd4', borderRadius: 12, padding: '1rem 1.25rem', marginBottom: '1rem' }}>
          ...
          {[
            ...
            <label key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', border: '0.5px solid #e0ddd4', borderRadius: 8, marginBottom: 6, cursor: 'pointer', fontSize: 13 }}>
              <input type="checkbox" checked={p.val} onChange={e => p.set(e.target.checked)} />
              {p.txt}
              <span style={{ marginLeft: 'auto', background: '#FCEBEB', color: '#501313', fontSize: 11, padding: '1px 7px', borderRadius: 8 }}>−10 pts</span>
            </label>
          ))}
      ...
      <div style={{ background: '#fff', border: '0.5px solid #e0ddd4', borderRadius: 12, padding: '1rem 1.25rem', marginBottom: '1rem' }}>
        <div style={{ fontSize: 13, fontWeight: 500, marginBottom: '0.75rem' }}>
          Crew del turno <span style={{ fontSize: 11, fontWeight: 400, color: '#888' }}>({crew.filter(c => !c.removido).length} personas)</span>
        </div>
        {crew.filter(c => !c.removido).length === 0 && (
          <div style={{ fontSize: 13, color: '#888', padding: '8px 0' }}>Sin crew asignado — subí el horario de Orquest</div>
        )}
        {crew.map((c, i) => !c.removido && (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '0.5px solid #f0ede6' }}>
            ...
            <div style={{ flex: 1, fontSize: 13 }}>
              {c.nombre}
              {(c.rol === 'entrenador') && <span style={{ ...badge('#EAF3DE', '#27500A'), marginLeft: 6 }}>entrenador</span>}
              {c.prop_turno < 1 && <span style={{ ...badge('#FAEEDA', '#633806'), marginLeft: 4 }}>{Math.round(c.prop_turno * 100)}% turno</span>}
      ...
      {guardado && <div style={{ background: '#EAF3DE', color: '#27500A', padding: '10px 14px', borderRadius: 8, fontSize: 13, marginBottom: '1rem' }}>✓ Turno guardado y puntos asignados al equipo</div>}
